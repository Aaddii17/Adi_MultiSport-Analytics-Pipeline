...
@st.cache_data
def load_data():
    # Force Pandas to handle dates at the root level to prevent type accessor errors
    df = pd.read_csv("data/ipl_ball_by_ball_2008_2025.csv", low_memory=False, parse_dates=['date'])
    return df

def run_ipl_analysis():
    st.header("🏏 Indian Premier League (IPL) Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Analyzing historical IPL ball-by-ball records..."):
        df = load_data()
        
    # Convert season column to string safely for filtering dropdowns
    ...
    # ==========================================
    # TAB 1: ORIGINAL LEADERBOARD & STATS
    # ==========================================
    with tab1:
        ...
        m2.metric("👑 Orange Cap Leader", f"{top_scorer}", f"{top_scorer_runs} Runs")
        m3.metric("🎯 Purple Cap Leader", f"{top_wicket_taker}", f"{top_wickets} Wickets")
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#FF9933", height=200) # Classic IPL Orange Theme
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#3F51B5", height=200) # Deep Purple-ish Blue
    ...
    with tab2:
        ...
        if not match_level_df.empty:
            # Sort matches chronologically and generate a clean label
            ...
            selected_match_str = st.selectbox("Choose Match", match_level_df['display_name'].tolist(), label_visibility="collapsed", key="ipl_match_selector")
            ...
            if selected_match_str:
                ...
                for innings_num in sorted(m_balls['innings'].unique()):
                    ...
                    # Generate Batting Card using your schema
                    ...
                    # Generate Bowling Card using your schema
