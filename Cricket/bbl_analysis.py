...
@st.cache_data
def load_data():
    df = pd.read_csv("data/bbl_combined.csv", low_memory=False)
    # Aggressively force datetime conversion to prevent format crashes
    df['date'] = pd.to_datetime(df['date'], format='mixed', errors='coerce')
    return df

def run_bbl_analysis():
    st.header("🦘 Big Bash League (BBL) Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Analyzing Big Bash records..."):
        df = load_data()
        
    df['season'] = df['season'].astype(str)
    
    st.write("🗓️ **Select BBL Season:**")
    seasons = sorted(df['season'].unique().tolist(), reverse=True)
    if "All-Time" not in seasons:
        seasons.insert(0, "All-Time")
        
    selected_season = st.selectbox("Season Dropdown", seasons, label_visibility="collapsed", key="bbl_season")
    ...
    with tab1:
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#00843D", height=200) # Aussie Green Hex
            
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#FFCD00", height=200) # Aussie Gold Hex

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR
    # ==========================================
    with tab2:
        st.subheader("🔍 Select a BBL Match to Inspect")
        ...
        if not match_level_df.empty:
            ...
            selected_match_str = st.selectbox("Choose Match", match_level_df['display_name'].tolist(), label_visibility="collapsed", key="bbl_match_selector")
