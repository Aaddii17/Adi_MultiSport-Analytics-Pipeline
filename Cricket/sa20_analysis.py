...
@st.cache_data
def load_data():
    # Force Pandas to handle dates at the root level
    df = pd.read_csv("data/sa20_combined.csv", low_memory=False, parse_dates=['date'])
    return df
...
def run_sa20_analysis():
    ...
    with tab1:
        ...
        with col_charts:
            ...
            st.bar_chart(bowler_wickets.head(10), color="#FFD700", height=200) # Gold Hex

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR
    # ==========================================
    with tab2:
        st.subheader("🔍 Select an SA20 Match to Inspect")
        ...
        if not match_level_df.empty:
            ...
            selected_match_str = st.selectbox("Choose Match", match_level_df['display_name'].tolist(), label_visibility="collapsed", key="sa20_match_selector")
