...
def prep_t20wc_data(input_folder, output_filename):
    print(f"🏆 Starting Men's T20 World Cup ETL Pipeline...")
    all_balls = []
    ...
    print("🧩 Merging World Cup data...")
    df = pd.DataFrame(all_balls)
    
    print("📅 Standardizing Tournament Years...")
    ...
    print(f"✅ T20 World Cup ETL Complete! Successfully processed {file_count} matches.")
    print(f"📊 Total deliveries processed: {len(df):,}")

if __name__ == "__main__":
    INPUT_DIR = "data/T20WC_Raw"
    OUTPUT_FILE = "data/t20wc_combined.csv"
    prep_t20wc_data(INPUT_DIR, OUTPUT_FILE)