...
def prep_wt20wc_data(input_folder, output_filename):
    print(f"🏆 Initializing Women's T20 World Cup (WT20WC) ETL Pipeline...")
    all_balls = []
    ...
    for filename in os.listdir(input_folder):
        ...
        match_info = {'match_id': match_id, 'teams': [], 'gender': 'female'} 
        balls_cache = []
        ...
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if row[0] == 'info':
                    ...
                    elif key == 'gender': match_info['gender'] = val
                    elif key == 'team': match_info['teams'].append(val)
                
                elif row[0] == 'ball':
                    balls_cache.append(row)
        
        # Safety check for Women's data
        if match_info.get('gender') != 'female':
            continue
            
        file_count += 1
        for row in balls_cache:
            batting_team = row[3]
    ...
    if not all_balls:
        print("⚠️ Error: No matches were processed! Please verify that your raw files are inside the 'data/WT20WC_Raw' folder.")
        return

    print("🧩 Merging data and formatting calendar years...")
    ...
    print(f"✅ WT20WC ETL Complete! Processed {file_count} matches successfully.")

if __name__ == "__main__":
    INPUT_DIR = "data/WT20WC_Raw" 
    OUTPUT_FILE = "data/wt20wc_combined.csv"
    prep_wt20wc_data(INPUT_DIR, OUTPUT_FILE)