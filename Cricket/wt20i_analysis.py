...
def run_wt20i_analysis():
    ...
    with st.spinner("Loading fast-paced WT20I history..."):
        df = load_data()
    ...
    selected_year = st.selectbox("Year Dropdown", years, label_visibility="collapsed", key="wt20i_year")
    ...
    with tab1:
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#FF007F", height=200) # Neon Pink
            
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#00E5FF", height=200) # Neon Cyan

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR (WITH TEAM FILTERS)
    # ==========================================
    with tab2:
        st.subheader("🔍 Filter & Select a WT20I Match")
        ...
        if not match_level_df.empty:
            ...
            with col_f1:
                team1_filter = st.selectbox("Filter by Team 1:", team_options, key="wt20i_t1")
            with col_f2:
                team2_filter = st.selectbox("Filter by Team 2:", team_options, key="wt20i_t2")
            ...
            if not filtered_matches.empty:
                ...
                selected_match_str = st.selectbox("Choose Match:", filtered_matches['display_name'].tolist(), label_visibility="collapsed", key="wt20i_match_selector")
