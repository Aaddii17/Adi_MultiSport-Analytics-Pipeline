...
@st.cache_data
def load_data():
    df = pd.read_csv("data/wodi_combined.csv", low_memory=False)
    df['date'] = pd.to_datetime(df['date'], format='mixed', errors='coerce')
    return df

def run_wodi_analysis():
    st.header("🌍 Women's One Day International (WODI) Hub")
    st.markdown("---")
    
    with st.spinner("Loading decades of WODI history..."):
        df = load_data()
    ...
    selected_year = st.selectbox("Year Dropdown", years, label_visibility="collapsed", key="wodi_year")
    ...
    with tab1:
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#0F4C81", height=200) # Classic Navy Blue
            
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#F57D7C", height=200) # Soft Coral

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR (WITH TEAM FILTERS)
    # ==========================================
    with tab2:
        st.subheader("🔍 Filter & Select a WODI Match")
        ...
        if not match_level_df.empty:
            ...
            with col_f1:
                team1_filter = st.selectbox("Filter by Team 1:", team_options, key="wodi_t1")
            with col_f2:
                team2_filter = st.selectbox("Filter by Team 2:", team_options, key="wodi_t2")
            ...
            if not filtered_matches.empty:
                ...
                selected_match_str = st.selectbox("Choose Match:", filtered_matches['display_name'].tolist(), label_visibility="collapsed", key="wodi_match_selector")
