...
def prep_sa20_data(input_folder, output_filename):
    print(f"🇿🇦 Upgrading SA20 ETL Pipeline for the Match Inspector...")
    all_balls = []
            ...
            for row in reader:
                ...
                if row[0] == 'info':
                    ...
                    ball_data = {
                        ...
                        # --- NEW COLUMNS ADDED FOR MATCH INSPECTOR ---
                        'innings': int(row[1]),
                        'ball': float(row[2]),
                        'batter': row[4],
                        'non_striker': row[5],
    ...
    print(f"✅ SA20 ETL Complete! Processed {file_count} matches with full scorecard data.")

if __name__ == "__main__":
    # Ensure this INPUT_DIR matches whatever folder name you used for SA20 previously
    INPUT_DIR = "data/SA20_Raw" 
    OUTPUT_FILE = "data/sa20_combined.csv"
    prep_sa20_data(INPUT_DIR, OUTPUT_FILE)