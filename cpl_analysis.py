...
@st.cache_data
def load_data():
    df = pd.read_csv("data/cpl_combined.csv", low_memory=False)
    # Aggressively force datetime conversion, overriding any weird CPL text formats
    df['date'] = pd.to_datetime(df['date'], format='mixed', errors='coerce')
    return df

def run_cpl_analysis():
    st.header("🌴 Caribbean Premier League (CPL) Analytics")
    st.markdown("---")
    
    with st.spinner("Analyzing Caribbean Premier League data..."):
        df = load_data()
        
    df['season'] = df['season'].astype(str)
    
    st.write("🗓️ **Select CPL Season:**")
    seasons = sorted(df['season'].unique().tolist(), reverse=True)
    if "All-Time" not in seasons:
        seasons.insert(0, "All-Time")
        
    selected_season = st.selectbox("Season Dropdown", seasons, label_visibility="collapsed", key="cpl_season")
    ...
    with tab1:
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#00A859", height=200) # Caribbean Green Hex
            
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#FFC000", height=200) # Vibrant Yellow Hex

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR
    # ==========================================
    with tab2:
        st.subheader("🔍 Select a CPL Match to Inspect")
        
        if not match_level_df.empty:
            match_level_df = match_level_df.sort_values(by='date', ascending=False)
            
            # Added .fillna('Unknown Date') to prevent crashes if a date was completely unreadable
            ...
            selected_match_str = st.selectbox("Choose Match", match_level_df['display_name'].tolist(), label_visibility="collapsed", key="cpl_match_selector")
            ...
            if selected_match_str:
                ...
                # --- THE DYNAMIC INNINGS GENERATOR ---
                if 'innings' not in m_balls.columns:
                    # Creates a new innings number every time the batting team switches
