...
def run_cpl_analysis():
    st.header("🌴 Caribbean Premier League (CPL) Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Loading CPL Data..."):
        df = load_data()
    ...
    if selected_season != "All-Time":
        # Ensure we are comparing strings since seasons might be parsed as ints or strings
        filtered_df = df[df['season'].astype(str) == selected_season]
    else:
        filtered_df = df
    ...
    # ==========================================
    # SPLIT LAYOUT: POINTS TABLE & STACKED CHARTS
    # ==========================================
    ...
    with col_charts:
        ...
        st.bar_chart(top_10_bat, color="#00AEEF", height=220) # Caribbean Blue
        
        st.subheader("🔥 Top 10 Wicket Takers")
        top_10_bowl = bowler_wickets.head(10)
        st.bar_chart(top_10_bowl, color="#F26522", height=220) # Sunrise Orange
