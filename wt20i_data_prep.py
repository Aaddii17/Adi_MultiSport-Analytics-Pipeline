...
def prep_wt20i_data(input_folder, output_filename):
    print(f"🌍 Initializing Women's T20 International (WT20I) ETL Pipeline...")
    all_match_dfs = []
    
    file_count = 0
    for filename in os.listdir(input_folder):
        # Process the ball-by-ball files, map the info file dynamically
        if filename.endswith(".csv") and not filename.endswith("_info.csv"):
            ...
            # Map Format B column names to our pipeline schema
            rename_map = {
                'start_date': 'date',
                'striker': 'batter',
            ...
            # Safe filter in case of corrupted raw files
            cols_present = [c for c in cols_to_keep if c in df.columns]
            df = df[cols_present]
            all_match_dfs.append(df)

    print("🧩 Merging almost 2,000 matches into master dataset...")
    ...
    print(f"✅ WT20I ETL Complete! Processed {file_count} matches seamlessly.")

if __name__ == "__main__":
    INPUT_DIR = "data/WT20I_Raw" 
    OUTPUT_FILE = "data/wt20i_combined.csv"
    prep_wt20i_data(INPUT_DIR, OUTPUT_FILE)