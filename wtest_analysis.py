...
@st.cache_data
def load_data():
    df = pd.read_csv("data/wtest_combined.csv", low_memory=False)
    df['date'] = pd.to_datetime(df['date'], format='mixed', errors='coerce')
    return df

def run_wtest_analysis():
    st.header("🏏 Women's Test International Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Loading centuries of Women's Test history..."):
        df = load_data()
    ...
    selected_year = st.selectbox("Year Dropdown", years, label_visibility="collapsed", key="wtest_year")
    ...
    with tab1:
        ...
        m1.metric("Tests Played", total_matches)
        ...
        with col_table:
            ...
            for team in all_teams:
                ...
                draws = len(match_level_df[((match_level_df['batting_team'] == team) | (match_level_df['bowling_team'] == team)) & (match_level_df['match_won_by'] == 'Draw')])
                losses = played - wins - draws
                win_pct = round((wins / played) * 100, 1) if played > 0 else 0.0
                points_data.append({'Team': team, 'Played': played, 'Wins': wins, 'Losses': losses, 'Draws': draws, 'Win %': win_pct})
                
            if points_data:
                points_df = pd.DataFrame(points_data).sort_values(by=['Wins', 'Win %'], ascending=[False, False])
                points_df.index = range(1, len(points_df) + 1)
                st.dataframe(points_df, use_container_width=True, height=400)
            else:
                st.info("No match data available to generate statistics.")
            
        with col_charts:
            st.subheader("🏏 Top 10 Run Scorers")
            st.bar_chart(batsman_runs.head(10), color="#990000", height=200) # Crimson
            
            st.subheader("🎯 Top 10 Wicket Takers")
            st.bar_chart(bowler_wickets.head(10), color="#D4AF37", height=200) # Gold

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR (WITH TEAM FILTERS)
    # ==========================================
    with tab2:
        st.subheader("🔍 Filter & Select a Women's Test Match")
        ...
        if not match_level_df.empty:
            ...
            with col_f1:
                team1_filter = st.selectbox("Filter by Team 1:", team_options, key="wtest_t1")
            with col_f2:
                team2_filter = st.selectbox("Filter by Team 2:", team_options, key="wtest_t2")
            ...
            if not filtered_matches.empty:
                ...
                selected_match_str = st.selectbox("Choose Match:", filtered_matches['display_name'].tolist(), label_visibility="collapsed", key="wtest_match_selector")
                ...
                if selected_match_str:
                    ...
                    st.success(f"🏟️ **Venue:** {m_info['venue']}  |  🏆 **Result/Winner:** {m_info['match_won_by']}  |  ⭐ **Player of the Match:** {m_info['player_of_match']}")
                    
                    # Test matches can iterate through 4 innings smoothly!
