...
def load_data():
    try:
        df = pd.read_csv("data/seriea_combined.csv", low_memory=False)
...
def run_seriea_analysis():
    st.header("🇮🇹 Serie A Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Loading Serie A history..."):
        df = load_data()
        
    st.write("🗓️ **Select Serie A Season:**")
    ...
    with tab1:
        ...
        with col_charts:
            if not league_table.empty:
                ...
                # Serie A Blue
                fig1 = px.bar(top_attack, x='Goals For', y='Team', orientation='h', color_discrete_sequence=['#0047AB'])
                ...
                # Serie A Gold
                fig2 = px.bar(top_defense, x='Goals Against', y='Team', orientation='h', color_discrete_sequence=['#D4AF37'])
        ...
        if filtered_df.empty:
            ...
            if selected_match_str:
                ...
                if not match_subset.empty:
                    ...
                    # DYNAMIC CHECK: Do advanced stats exist for this specific match?
                    # If total shots and total corners equal 0, we assume it's a basic historical file.
                    advanced_stats_exist = (m.get('hs', 0) + m.get('as', 0) + m.get('hc', 0) + m.get('ac', 0)) > 0
                    
                    if advanced_stats_exist:
                        # FULL BROADCAST LAYOUT
                        st.markdown(f"""
                        <div style='background-color: #0047AB; padding: 15px; border-radius: 10px; margin-bottom: 15px;'>
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 28px;'>{m['home_team']} <span style='color: #D4AF37;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}</h2>
                            <p style='text-align: center; color: #D4AF37; font-size: 14px; margin: 0; padding-top: 5px; font-weight: bold;'>HALF-TIME: {int(m['hthg'])} - {int(m['htag'])}</p>
                        </div>
                        ...
                        def stat_row(home_stat, stat_name, away_stat):
                            ...
                            <table style="width: 100%; background-color: transparent; border-bottom: 1px solid #333; margin-bottom: 0px;">
                                <tr>
                                    ...
                                    <td style="width: 34%; text-align: center; color: #D4AF37; font-size: 14px; font-weight: bold; white-space: nowrap; padding: 8px 0; border: none; letter-spacing: 1px;">{stat_name.upper()}</td>
                        ...
                        st.markdown("<hr style='border: 1.5px solid #D4AF37; margin-top: 0; margin-bottom: 10px;'>", unsafe_allow_html=True)
                        ...
                        # MINIMAL HISTORICAL LAYOUT
                        st.markdown(f"""
                        <div style='background-color: #1A1A1A; padding: 30px; border-radius: 10px; margin-top: 20px; border-left: 6px solid #0047AB;'>
                            <p style='text-align: center; color: #D4AF37; font-size: 14px; margin: 0; padding-bottom: 10px; font-weight: bold; letter-spacing: 2px;'>FULL TIME RESULT • {date_display.upper()}</p>
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 38px;'>
                                {m['home_team']} <span style='color: #0047AB; padding: 0 20px;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}
                            </h2>
