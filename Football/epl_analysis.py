...
def load_data():
    try:
        df = pd.read_csv("data/epl_combined.csv", low_memory=False)
...
def generate_league_table(df):
    # Updated columns for full readability
    if df.empty:
    ...
    # Sorting logic updated with new names
    res_df = pd.DataFrame(table).sort_values(by=['Points', 'Goal Difference', 'Goals For'], ascending=[False, False, False])
    res_df.index = range(1, len(res_df) + 1)
    return res_df

def run_epl_analysis():
    st.header("🦁 English Premier League Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Loading Premier League history..."):
        df = load_data()
        
    st.write("🗓️ **Select EPL Season:**")
    ...
    with tab1:
        ...
        with col_table:
            ...
            st.dataframe(league_table, use_container_width=True, height=500)
            
        with col_charts:
            if not league_table.empty:
                # Updated chart logic for the new readable column names
                ...
                fig1 = px.bar(top_attack, x='Goals For', y='Team', orientation='h', color_discrete_sequence=['#38003C'])
                ...
                fig2 = px.bar(top_defense, x='Goals Against', y='Team', orientation='h', color_discrete_sequence=['#E90052'])
    ...
    # ==========================================
    # TAB 2: BROADCAST-STYLE MATCH INSPECTOR
    # ==========================================
    with tab2:
        ...
        if filtered_df.empty:
            ...
            if selected_match_str:
                ...
                if not match_subset.empty:
                    ...
                    st.markdown(f"""
                    <div style='background-color: #38003C; padding: 15px; border-radius: 10px; margin-bottom: 15px;'>
                        <h2 style='text-align: center; color: white; margin: 0; font-size: 28px;'>{m['home_team']} <span style='color: #E90052;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}</h2>
                        <p style='text-align: center; color: #00FF85; font-size: 14px; margin: 0; padding-top: 5px; font-weight: bold;'>HALF-TIME: {int(m['hthg'])} - {int(m['htag'])}</p>
                    </div>
                    ...
                    def stat_row(home_stat, stat_name, away_stat):
                        ...
                        <table style="width: 100%; background-color: transparent; border-bottom: 1px solid #333; margin-bottom: 0px;">
                            <tr>
                                ...
                                <td style="width: 34%; text-align: center; color: #00FF85; font-size: 14px; font-weight: bold; white-space: nowrap; padding: 8px 0; border: none; letter-spacing: 1px;">{stat_name.upper()}</td>
                    ...
                    st.markdown("<hr style='border: 1.5px solid #E90052; margin-top: 0; margin-bottom: 10px;'>", unsafe_allow_html=True)
