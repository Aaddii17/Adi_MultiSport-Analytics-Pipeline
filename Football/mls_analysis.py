...
def load_data():
    try:
        df = pd.read_csv("data/mls_combined.csv", low_memory=False)
...
def run_mls_analysis():
    st.header("🇺🇸 Major League Soccer (MLS) Hub")
    st.markdown("---")
    
    with st.spinner("Loading MLS history..."):
        df = load_data()
        
    st.write("🗓️ **Select MLS Season:**")
    ...
    with tab1:
        ...
        with col_charts:
            if not league_table.empty:
                ...
                # MLS Navy
                fig1 = px.bar(top_attack, x='Goals For', y='Team', orientation='h', color_discrete_sequence=['#002D55'])
                ...
                # MLS Red
                fig2 = px.bar(top_defense, x='Goals Against', y='Team', orientation='h', color_discrete_sequence=['#E31837'])
            ...
            if selected_match_str:
                ...
                if not match_subset.empty:
                    ...
                    if advanced_stats_exist:
                        st.markdown(f"""
                        <div style='background-color: #002D55; padding: 15px; border-radius: 10px; margin-bottom: 15px;'>
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 28px;'>{m['home_team']} <span style='color: #E31837;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}</h2>
                        ...
                        def stat_row(home_stat, stat_name, away_stat):
                            ...
                            <table style="width: 100%; background-color: transparent; border-bottom: 1px solid #333; margin-bottom: 0px;">
                                <tr>
                                    ...
                                    <td style="width: 34%; text-align: center; color: #E31837; font-size: 14px; font-weight: bold; white-space: nowrap; padding: 8px 0; border: none; letter-spacing: 1px;">{stat_name.upper()}</td>
                        ...
                        st.markdown("<hr style='border: 1.5px solid #002D55; margin-top: 0; margin-bottom: 10px;'>", unsafe_allow_html=True)
                        ...
                        st.markdown(f"""
                        <div style='background-color: #1A1A1A; padding: 30px; border-radius: 10px; margin-top: 20px; border-left: 6px solid #002D55;'>
                            ...
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 38px;'>
                                {m['home_team']} <span style='color: #E31837; padding: 0 20px;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}
                            </h2>
