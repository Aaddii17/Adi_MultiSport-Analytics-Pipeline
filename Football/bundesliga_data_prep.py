...
def extract_season(filename):
    """Extracts season from filenames like 'Bundesliga 2012 - 2013.csv' and formats to '2012/13'"""
...
def prep_bundesliga_data(input_folder, output_file):
    print("⚽ Initializing Bundesliga ETL Pipeline...")
    
    file_pattern = os.path.join(input_folder, "bundesliga", "*.csv")
    all_files = glob.glob(file_pattern)

    if not all_files:
        print(f"⚠️ Error: No Bundesliga files found in {input_folder}bundesliga/")
        return
    ...
    for file in all_files:
        ...
        # FIX: Added on_bad_lines='skip' to automatically drop corrupted rows with stray commas
        try:
    ...
    print(f"✅ Bundesliga ETL Complete! Total Matches Processed: {len(final_df)}")

if __name__ == "__main__":
    DATA_DIR = "../data/" if os.path.exists("../data/") else "data/"
    OUTPUT_FILE = os.path.join(DATA_DIR, "bundesliga_combined.csv")
    prep_bundesliga_data(DATA_DIR, OUTPUT_FILE)