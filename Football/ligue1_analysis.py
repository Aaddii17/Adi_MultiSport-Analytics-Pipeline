...
def load_data():
    try:
        df = pd.read_csv("data/ligue1_combined.csv", low_memory=False)
...
def run_ligue1_analysis():
    st.header("🇫🇷 Ligue 1 Analytics Hub")
    st.markdown("---")
    
    with st.spinner("Loading Ligue 1 history..."):
        df = load_data()
        
    st.write("🗓️ **Select Ligue 1 Season:**")
    ...
    with tab1:
        ...
        with col_charts:
            if not league_table.empty:
                ...
                # Ligue 1 Modern Navy
                fig1 = px.bar(top_attack, x='Goals For', y='Team', orientation='h', color_discrete_sequence=['#0A1C56'])
                ...
                # Ligue 1 Neon Lime
                fig2 = px.bar(top_defense, x='Goals Against', y='Team', orientation='h', color_discrete_sequence=['#CCFF00'])
            ...
            if selected_match_str:
                ...
                if not match_subset.empty:
                    ...
                    if advanced_stats_exist:
                        st.markdown(f"""
                        <div style='background-color: #0A1C56; padding: 15px; border-radius: 10px; margin-bottom: 15px;'>
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 28px;'>{m['home_team']} <span style='color: #CCFF00;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}</h2>
                        ...
                        def stat_row(home_stat, stat_name, away_stat):
                            ...
                            <table style="width: 100%; background-color: transparent; border-bottom: 1px solid #333; margin-bottom: 0px;">
                                <tr>
                                    ...
                                    <td style="width: 34%; text-align: center; color: #CCFF00; font-size: 14px; font-weight: bold; white-space: nowrap; padding: 8px 0; border: none; letter-spacing: 1px;">{stat_name.upper()}</td>
                        ...
                        st.markdown("<hr style='border: 1.5px solid #0A1C56; margin-top: 0; margin-bottom: 10px;'>", unsafe_allow_html=True)
                        ...
                        st.markdown(f"""
                        <div style='background-color: #1A1A1A; padding: 30px; border-radius: 10px; margin-top: 20px; border-left: 6px solid #0A1C56;'>
                            ...
                            <h2 style='text-align: center; color: white; margin: 0; font-size: 38px;'>
                                {m['home_team']} <span style='color: #0A1C56; padding: 0 20px;'>{int(m['fthg'])} - {int(m['ftag'])}</span> {m['away_team']}
                            </h2>
