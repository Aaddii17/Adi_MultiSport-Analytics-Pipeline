...
@st.cache_data
def load_data():
    df = pd.read_csv("data/wbbl_combined.csv", low_memory=False)
    # Safely handle dates to prevent crashes
    df['date'] = pd.to_datetime(df['date'], format='mixed', errors='coerce')
    return df

def run_wbbl_analysis():
    st.header("🦘 Women's Big Bash League (WBBL) Analytics")
    st.markdown("---")
    
    with st.spinner("Analyzing WBBL historical records..."):
        df = load_data()
        
    df['season'] = df['season'].astype(str)
    
    st.write("🗓️ **Select WBBL Season:**")
    seasons = sorted(df['season'].unique().tolist(), reverse=True)
    if "All-Time" not in seasons:
        seasons.insert(0, "All-Time")
        
    selected_season = st.selectbox("Season Dropdown", seasons, label_visibility="collapsed", key="wbbl_season")
    ...
    with tab1:
        ...
        with col_table:
            ...
            st.subheader("🥇 WBBL All-Time Champions")
            history_data = {
                'Team': ['Brisbane Heat', 'Sydney Sixers', 'Sydney Thunder', 'Adelaide Strikers', 'Perth Scorchers'],
                'Titles Won': [3, 2, 2, 2, 2],
                'Winning Years': [
        ...
        with col_charts:
            ...
            st.bar_chart(batsman_runs.head(10), color="#00FFFF", height=200) # Cyan Hex
            
            st.subheader("🎯 Top 10 Bowling Performances")
            st.bar_chart(bowler_wickets.head(10), color="#FF00FF", height=200) # Magenta Hex

    # ==========================================
    # TAB 2: MATCH SCORECARD INSPECTOR
    # ==========================================
    with tab2:
        st.subheader("🔍 Select a WBBL Match to Inspect")
        ...
        if not match_level_df.empty:
            ...
            selected_match_str = st.selectbox("Choose Match", match_level_df['display_name'].tolist(), label_visibility="collapsed", key="wbbl_match_selector")
